...
Production runs the multi-stage `Dockerfile` (Node 24 Alpine runtime). The container CMD is `npm run start:migrate`, which applies pending Prisma migrations (`migrate deploy`, a no-op when current) before serving, so plain Docker hosts self-migrate on boot. Railway does not migrate on boot: the start command in `.railway/railway.ts` replaces the CMD and only serves, and migrations run once per deploy as the pre-deploy command, so a failed migration fails that deploy (the current release keeps serving) instead of crash-looping the container.
